...
import { setRefreshTokenCookie, clearRefreshTokenCookie } from "@/src/app/actions";
...
import { HabitsData } from "@/src/lib/googleSheets/googleSheetsApi";
...
type Props = {
  // Pre-fetched data from the Server Component (may be null if not authenticated)
  initialData: (HabitsData & { spreadsheetId?: string; spreadsheetUrl?: string }) | null;
};

export default function HabitsClient({ initialData }: Props) {
  ...
  // Initialize Redux with server-pre-fetched data (no HTTP round trip needed).
  // Runs after token is restored so SWR doesn't overwrite immediately.
  useEffect(() => {
    if (!initialData || !today) return;
    ...
    if (initialData.spreadsheetId) dispatch(setSpreadsheetId(initialData.spreadsheetId));
    if (initialData.spreadsheetUrl) dispatch(setSpreadsheetUrl(initialData.spreadsheetUrl));
    dispatch(setStatus(GoogleState.CONNECTED));
  }, [initialData, today, dispatch]);
  ...
  const handleSetRefreshToken = useCallback(
    (token: string) => {
      ...
      // Also update httpOnly cookie so the Server Component can pre-fetch on next load
      if (token) {
